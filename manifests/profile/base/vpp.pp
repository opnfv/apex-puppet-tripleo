...
class tripleo::profile::base::vpp (
  ...
  if $step >= 1 {
    ...
    if !empty($vpp_ctlplane_cidr) and size($controllers) == 1 {
      ...
      exec { 'vpp admin - vpp config':
        command => "vppctl create host-interface name vpp-admin && vppctl set int ip addr host-vpp-admin ${vpp_ctlplane_cidr} && vppctl set interface state host-vpp-admin up",
        path    => ['/bin', '/sbin'],
        unless  => "vppctl show int host-vpp-admin | grep -E 'host-vpp-admin +[0-9]+'",
        tries   => 5,
        timeout => 15,
        require => Exec['vpp admin - kernel interface config'],
      }
    }
  }
}
